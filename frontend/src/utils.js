...
export default class Utils {
  ...
  // Subscriber import fields stored in attribs JSON.
  subscriberAttrib = (attribs, key) => {
    if (!attribs || attribs[key] === undefined || attribs[key] === null) {
      return '';
    }
    return String(attribs[key]).trim();
  };

  subscriberAttribsForEditor = (attribs) => {
    if (!attribs) {
      return {};
    }
    const o = { ...attribs };
    ['firstname', 'lastname', 'company'].forEach((k) => delete o[k]);
    return o;
  };

  mergeSubscriberAttribs = (attribs, { firstname, lastname, company }) => {
    const merged = { ...(attribs || {}) };
    const set = (k, v) => {
      const t = (v || '').trim();
      if (t) {
        merged[k] = t;
