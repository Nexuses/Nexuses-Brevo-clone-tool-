    ...
    <b-table :data="campaigns.results" :loading="loading.campaigns" :row-class="highlightedRow"
      ...
      <b-table-column v-slot="props" cell-class="actions" width="15%" align="right">
        <div>
          ...
          <a v-if="$can('campaigns:get_analytics') && canDownloadReport(props.row)"
            :href="`/api/campaigns/${props.row.id}/report`" target="_blank" rel="noopener noreferrer"
            data-cy="btn-campaign-report" :aria-label="$t('campaigns.downloadReport')">
