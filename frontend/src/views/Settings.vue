      ...
      <section class="settings-brevo__body wrap" v-if="form">
        <b-tabs :animated="false" v-model="tab">
          ...
          <b-tab-item :label="$t('settings.smtp.name')">
            ...
            <cloudflare-settings :form="form" :key="'cf-' + key" />
...
import CloudflareSettings from './settings/cloudflare.vue';
...
export default Vue.extend({
  ...
  methods: {
    async onSubmit() {
      ...
      } else if (this.isDummy(form.cloudflare_email.api_token)) {
        form.cloudflare_email.api_token = '';
      } else if (this.hasDummy(form.cloudflare_email.api_token)) {
        hasDummy = 'cloudflare';
      }
