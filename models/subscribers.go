...
func (s Subscriber) attribsString(key string) string {
	if s.Attribs == nil {
		return ""
	}
	v, ok := s.Attribs[key]
...
// FirstName returns the imported first name when present; otherwise it splits
// the display name and returns the first chunk greater than 2 characters.
func (s Subscriber) FirstName() string {
	if v := s.attribsString("firstname"); v != "" {
		return v
	}
...
// LastName returns the imported last name when present; otherwise it splits
// the display name and returns the last chunk greater than 2 characters.
func (s Subscriber) LastName() string {
	if v := s.attribsString("lastname"); v != "" {
		return v
	}
...
// Company returns the imported company name when present.
func (s Subscriber) Company() string {
	return s.attribsString("company")
}
