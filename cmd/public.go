...
//
// Host validation for custom tracking domains is intentionally not enforced here:
// listmonk commonly runs behind reverse proxies that rewrite Host, and local/dev
// setups often hit the platform host while links still use unpredictable link,
// campaign, and subscriber UUIDs. Rejecting on Host would break legitimate traffic.
func (a *App) LinkRedirect(c echo.Context) error {
	...
	if err != nil {
		if e, ok := err.(*echo.HTTPError); ok {
			...
			return c.Render(e.Code, tplMessage, makeMsgTpl(a.i18n.T("public.errorTitle"), "", msg))
		}
		return err
	}

	// If tracking is disabled globally or for this campaign, resolve without recording.
	if a.cfg.Privacy.DisableTracking || !camp.TrackClicks(a.cfg.Privacy.DisableTracking) {
	...
	subUUID = trackingSubscriberUUID(subUUID)
...
func (a *App) RegisterCampaignView(c echo.Context) error {
	...
	if campUUID != dummyUUID && !a.cfg.Privacy.DisableTracking {
		if camp, err := a.core.GetCampaign(0, campUUID, ""); err == nil && camp.TrackViews(a.cfg.Privacy.DisableTracking) {
			if err := a.core.RegisterCampaignView(campUUID, trackingSubscriberUUID(subUUID)); err != nil {
