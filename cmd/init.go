...
// initCloudflareMessenger initializes the Cloudflare Email Sending messenger
// when it is enabled and credentials are present.
func initCloudflareMessenger(ko *koanf.Koanf) []manager.Messenger {
	if !ko.Bool("cloudflare_email.enabled") {
		return nil
	}

	var o cloudflare.Options
	if err := ko.UnmarshalWithConf("cloudflare_email", &o, koanf.UnmarshalConf{Tag: "json"}); err != nil {
		lo.Fatalf("error reading Cloudflare Email config: %v", err)
	}

	m, err := cloudflare.New(o)
	if err != nil {
		lo.Fatalf("error initializing Cloudflare Email messenger: %v", err)
	}

	lo.Printf("initialized Cloudflare Email messenger")
	return []manager.Messenger{m}
}
