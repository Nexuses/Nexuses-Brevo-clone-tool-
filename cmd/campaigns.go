...
import (
	...
	"github.com/knadh/listmonk/internal/reports"
...
func (a *App) PreviewCampaign(c echo.Context) error {
	...
	// For visual/maily content, template ID for previewing is irrelevant.
	if contentType == models.CampaignContentTypeVisual ||
		contentType == models.CampaignContentTypeMaily || tplID < 1 {
		tplID = 0
	}
	...
	if isPost {
		...
		// For visual/maily campaigns, template body from the DB shouldn't be used.
		if contentType == models.CampaignContentTypeVisual ||
			contentType == models.CampaignContentTypeMaily {
			camp.TemplateBody = ""
		}
	}
...
func (a *App) TestCampaign(c echo.Context) error {
	...
	// Use the template from the request body; fall back to form value or the saved campaign template.
	tplID := 0
	if req.TemplateID.Valid && req.TemplateID.Int > 0 {
		tplID = req.TemplateID.Int
	}
	if tplID < 1 {
		tplID, _ = strconv.Atoi(c.FormValue("template_id"))
	}
	...
	if len(req.TrackingConfig) > 0 {
		camp.TrackingConfig = req.TrackingConfig
	}
...
// GetCampaignAnalyticsRecords returns paginated view or click records with subscriber details.
func (a *App) GetCampaignAnalyticsRecords(c echo.Context) error {
	ids, err := parseStringIDs(c.Request().URL.Query()["id"])
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest,
			a.i18n.Ts("globals.messages.errorInvalidIDs", "error", err.Error()))
	...
	out, total, err := a.core.QueryCampaignAnalyticsRecords(ids, typ, from, to, pg.Offset, pg.Limit)
	if err != nil {
		return err
	}
...
func (a *App) ExportCampaignReport(c echo.Context) error {
	...
	data, err := a.core.GetCampaignReportData(id)
	if err != nil {
		return err
	}

	out, err := reports.GenerateCampaignXLSX(data)
	if err != nil {
		a.log.Printf("error generating campaign report: %v", err)
		return echo.NewHTTPError(http.StatusInternalServerError, a.i18n.Ts("globals.messages.internalError"))
	}

	filename := fmt.Sprintf("campaign-report-%d.xlsx", id)
	c.Response().Header().Set(echo.HeaderContentType, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	c.Response().Header().Set(echo.HeaderContentDisposition, fmt.Sprintf(`attachment; filename="%s"`, filename))
	c.Response().Header().Set("Cache-Control", "no-cache")
	return c.Blob(http.StatusOK, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", out)
}
...
func (a *App) validateCampaignFields(c campReq) (campReq, error) {
	...
	if c.ReplyTo != "" && !reFromAddress.Match([]byte(c.ReplyTo)) {
		if _, err := a.importer.SanitizeEmail(c.ReplyTo); err != nil {
	...
	if c.ContentType != models.CampaignContentTypeRichtext &&
		...
		c.ContentType != models.CampaignContentTypeMaily &&
