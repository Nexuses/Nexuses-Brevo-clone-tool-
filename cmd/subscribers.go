...
// trackingSubscriberUUID returns the subscriber UUID to store for a tracking
// event. Aggregate tracking embeds dummyUUID in message URLs; per-subscriber
// tracking embeds the real UUID at send time. Honor the UUID in the request
// when it is not the dummy placeholder.
func trackingSubscriberUUID(subUUID string) string {
	if subUUID == "" || subUUID == dummyUUID {
		return ""
	}
	return subUUID
}
