...
func installTemplates(q *models.Queries) (int, int) {
	...
	plainTpl, err := fs.Get("/static/email-templates/plain.tpl")
	if err != nil {
		lo.Fatalf("error reading plain e-mail template: %v", err)
	}
	if _, err := q.CreateTemplate.Exec("Plain (1:1 email)", models.TemplateTypeCampaign, "", plainTpl.ReadBytes(), nil); err != nil {
		lo.Fatalf("error creating plain campaign template: %v", err)
	}
	...
	if _, err := q.CreateTemplate.Exec("Sample visual template", models.TemplateTypeCampaignVisual, "", visualTpl.ReadBytes(), visualSrc.ReadBytes()); err != nil {
		lo.Fatalf("error creating sample visual template: %v", err)
	}

	mailySrc, err := fs.Get("/static/email-templates/default-maily.json")
	if err != nil {
		lo.Fatalf("error reading default maily template json: %v", err)
	}
	if _, err := q.CreateTemplate.Exec("Sample Maily template", models.TemplateTypeCampaignMaily, "",
		[]byte(`<p>Hello {{ .Subscriber.Name }}</p>`), mailySrc.ReadBytes()); err != nil {
		lo.Fatalf("error creating sample maily template: %v", err)
	}
