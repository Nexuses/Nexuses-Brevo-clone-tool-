...
func (a *App) GetSettings(c echo.Context) error {
	...
	s.CloudflareEmail.APIToken = strings.Repeat(pwdMask, utf8.RuneCountInString(s.CloudflareEmail.APIToken))
...
func (a *App) UpdateSettings(c echo.Context) error {
	...
	// Duplicates are disallowed; "email" and "cloudflare" are reserved names.
	names := map[string]bool{emailMsgr: true, cloudflare.MessengerName: true}
	...
	if set.CloudflareEmail.APIToken == "" {
		set.CloudflareEmail.APIToken = cur.CloudflareEmail.APIToken
	}
	set.CloudflareEmail.AccountID = strings.TrimSpace(set.CloudflareEmail.AccountID)
	if set.CloudflareEmail.Enabled {
		if set.CloudflareEmail.AccountID == "" || set.CloudflareEmail.APIToken == "" {
			return echo.NewHTTPError(http.StatusBadRequest,
				a.i18n.Ts("globals.messages.missingFields", "name", "cloudflare account_id / api_token"))
		}
	}
...
// TestCloudflareSettings sends a test e-mail via the Cloudflare Email Sending API
// using the credentials posted from the settings UI.
func (a *App) TestCloudflareSettings(c echo.Context) error {
	type req struct {
	...
	if from == "" {
		return echo.NewHTTPError(http.StatusBadRequest, a.i18n.Ts("globals.messages.missingFields", "name", "from"))
	}
	if r.AccountID == "" || r.APIToken == "" || strings.Contains(r.APIToken, pwdMask) {
		return echo.NewHTTPError(http.StatusBadRequest, a.i18n.T("settings.cloudflare.testEnterToken"))
	}
	...
	msgr, err := cloudflare.New(cloudflare.Options{
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest,
			a.i18n.Ts("globals.messages.errorCreating", "name", "Cloudflare Email", "error", err.Error()))
	}
	...
	m := models.Message{
		From:    from,
		To:      []string{r.Email},
		Subject: a.i18n.T("settings.cloudflare.testConnection"),
		Body:    b.Bytes(),
	}
	if err := msgr.Push(m); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
