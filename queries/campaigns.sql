...
WITH tpl AS (
    ...
    SELECT
        -- If the template is a visual/maily template, then use it's HTML body as the campaign
        ...
        (CASE WHEN type IN ('campaign_visual', 'campaign_maily') THEN NULL ELSE id END) AS id,
        (CASE WHEN type IN ('campaign_visual', 'campaign_maily') THEN body ELSE '' END) AS body,
        (CASE WHEN type IN ('campaign_visual', 'campaign_maily') THEN body_source ELSE NULL END) AS body_source,
        (CASE WHEN type = 'campaign_visual' THEN 'visual'::content_type
              WHEN type = 'campaign_maily' THEN 'maily'::content_type
              ELSE 'richtext'::content_type END) AS content_type
    FROM templates
    WHERE
        CASE
            -- If a template ID is present, use it. If not, use the default template only if
            -- it's not a block-editor template.
            WHEN $14::INT IS NOT NULL THEN id = $14::INT
            ELSE $8 NOT IN ('visual', 'maily') AND is_default = TRUE
        END
    LIMIT 1
),
camp AS (
...
med AS (
    INSERT INTO campaign_media (campaign_id, media_id, filename)
        (SELECT (SELECT id FROM camp), id, filename FROM media WHERE id=ANY($21::INT[]))
),
insLists AS (
...
WHERE ($1 = 0 OR id = $1)
    ...
    AND ($4 = '' OR
        (TRIM(BOTH '%' FROM $4) ~ '^[0-9]+$' AND c.id = TRIM(BOTH '%' FROM $4)::INT) OR
        TO_TSVECTOR(CONCAT(name, ' ', subject)) @@ TO_TSQUERY(regexp_replace(TRIM(BOTH '%' FROM $4), '\s+', '&', 'g')) OR
        CONCAT(c.name, ' ', c.subject) ILIKE REPLACE($4, '&', '%') OR
        CONCAT(c.name, ' ', c.subject) ILIKE $4
    )
...
-- name: query-campaign-view-records-count
SELECT COUNT(*) FROM campaign_views
    WHERE campaign_id = ANY($1) AND created_at >= $2 AND created_at <= $3;

-- name: query-campaign-view-records
SELECT campaign_views.campaign_id,
    ...
    WHERE campaign_views.campaign_id = ANY($1)
      AND campaign_views.created_at >= $2 AND campaign_views.created_at <= $3
    ORDER BY campaign_views.created_at DESC
    LIMIT $4 OFFSET $5;

-- name: query-campaign-click-records-count
SELECT COUNT(*) FROM link_clicks
    WHERE campaign_id = ANY($1) AND created_at >= $2 AND created_at <= $3;

-- name: query-campaign-click-records
SELECT link_clicks.campaign_id,
    ...
    WHERE link_clicks.campaign_id = ANY($1)
      AND link_clicks.created_at >= $2 AND link_clicks.created_at <= $3
...
WITH camp AS (
    UPDATE campaigns SET
        ...
        tags=$11::VARCHAR(100)[],
        messenger=$12,
        -- template_id shouldn't be saved for visual/maily campaigns.
        template_id=(CASE WHEN $7::content_type IN ('visual', 'maily') THEN NULL ELSE $13::INT END),
        archive=$15,
        archive_slug=$16,
        archive_template_id=(CASE WHEN $7::content_type IN ('visual', 'maily') THEN NULL ELSE $17::INT END),
        archive_meta=$18,
        tracking_config=COALESCE($20, '{}'::JSONB),
        body_source=$21,
        updated_at=NOW()
...
WHERE (
    CASE
        ...
        ELSE $2 = '' OR
            (TRIM(BOTH '%' FROM $2) ~ '^[0-9]+$' AND c.id = TRIM(BOTH '%' FROM $2)::INT) OR
            TO_TSVECTOR(CONCAT(name, ' ', subject)) @@ TO_TSQUERY(regexp_replace(TRIM(BOTH '%' FROM $2), '\s+', '&', 'g')) OR
            CONCAT(c.name, ' ', c.subject) ILIKE REPLACE($2, '&', '%') OR
            CONCAT(c.name, ' ', c.subject) ILIKE $2
    END
)
...
-- name: get-campaign-report-audience
...
ORDER BY s.id, s.email;

-- name: get-campaign-report-view-stats
SELECT subscriber_id,
       COUNT(*) AS opens,
       MIN(created_at) AS first_open_at
FROM campaign_views
WHERE campaign_id = $1 AND subscriber_id IS NOT NULL
GROUP BY subscriber_id;

-- name: get-campaign-report-click-stats
SELECT link_clicks.subscriber_id,
       COUNT(*) AS clicks,
       MIN(link_clicks.created_at) AS first_click_at,
       (ARRAY_AGG(links.url ORDER BY link_clicks.created_at ASC))[1] AS first_click_url
FROM link_clicks
LEFT JOIN links ON links.id = link_clicks.link_id
WHERE link_clicks.campaign_id = $1 AND link_clicks.subscriber_id IS NOT NULL
GROUP BY link_clicks.subscriber_id;

-- name: get-campaign-report-bounces
SELECT b.type,
       b.source,
       b.meta,
       b.created_at,
       ...
       COALESCE(s.attribs, '{}') AS attribs
FROM bounces b
LEFT JOIN subscribers s ON s.id = b.subscriber_id
WHERE b.campaign_id = $1
ORDER BY b.created_at ASC;

-- name: get-campaign-report-unsub-count
SELECT COUNT(DISTINCT sl.subscriber_id)
FROM subscriber_lists sl
JOIN campaign_lists cl ON cl.list_id = sl.list_id
WHERE cl.campaign_id = $1 AND sl.status = 'unsubscribed';

-- name: get-campaign-report-unique-counts
SELECT
