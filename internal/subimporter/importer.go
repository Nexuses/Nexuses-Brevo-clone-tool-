...
func (s *Session) LoadCSV(srcPath string, delim rune) error {
	...
	if len(hdrKeys) > 0 {
		keys := make([]string, 0, len(hdrKeys))
		for k := range hdrKeys {
			keys = append(keys, k)
		}
		sort.Strings(keys)
		s.log.Printf("recognized CSV columns: %s", strings.Join(keys, ", "))
	}
...
// applyCSVFields maps dedicated CSV columns (name, firstname, lastname, company, attributes)
// onto the subscriber record before validation.
func (s *Session) applyCSVFields(sub *SubReq, row map[string]string, lineNum int) {
	if v, ok := row["name"]; ok {
		sub.Name = v
	}
	...
	sub.Name = composeSubscriberName(sub.Name, first, last)

	if first == "" && last == "" {
		first, last = splitSubscriberName(sub.Name)
	}

	if len(row["attributes"]) > 0 {
		var attribs models.JSON
		if err := json.Unmarshal([]byte(row["attributes"]), &attribs); err != nil {
			s.log.Printf("skipping invalid attributes JSON on line %d for '%s': %v", lineNum, sub.Email, err)
		} else {
			sub.Attribs = attribs
			if company == "" {
				company = csvAttribString(attribs, "company")
			}
			if first == "" {
				first = csvAttribString(attribs, "firstname")
			}
			if last == "" {
				last = csvAttribString(attribs, "lastname")
			}
		}
	}

	setImportedAttribFields(sub, first, last, company)
}

func csvAttribString(attribs models.JSON, key string) string {
	if attribs == nil {
		return ""
	}
	v, ok := attribs[key]
...
func setImportedAttribFields(sub *SubReq, first, last, company string) {
	if sub.Attribs == nil {
		sub.Attribs = models.JSON{}
	}
	if first != "" {
		sub.Attribs["firstname"] = first
	}
	if last != "" {
		sub.Attribs["lastname"] = last
	}
	if company != "" {
		sub.Attribs["company"] = company
	}
}

// composeSubscriberName returns an explicit name if set; otherwise joins first and last name.
func composeSubscriberName(name, first, last string) string {
...
// splitSubscriberName derives first/last name parts from a display name.
...
// canonicalCSVHeader maps common spreadsheet header variants to listmonk import keys.
func canonicalCSVHeader(h string) string {
...
func (s *Session) mapCSVHeaders(csvHdrs []string, knownHdrs map[string]bool) map[string]int {
	...
	for i, h := range csvHdrs {
		...
		raw := regexCleanStr.ReplaceAllString(strings.TrimSpace(h), "")
		canon := canonicalCSVHeader(raw)
		if canon == "" || !knownHdrs[canon] {
			s.log.Printf("ignoring unknown header '%s'", raw)
			continue
		}
		hdrKeys[canon] = i
	}
