...
func (m *Manager) TemplateFuncs(c *models.Campaign) template.FuncMap {
	f := template.FuncMap{
		"TrackLink": func(url string, msg *CampaignMessage) string {
			if m.cfg.DisableTracking || !msg.Campaign.TrackClicks(m.cfg.DisableTracking) {
				return msg.Campaign.ApplyTrackingParams(url)
			}
...
func (m *Manager) worker() {
	...
	for {
		...
		case msg, ok := <-m.campMsgQ:
			...
			// Reply-To: dedicated field wins; otherwise keep custom header or fall back to From.
			if msg.Campaign.ReplyTo != "" {
				h.Set("Reply-To", msg.Campaign.ReplyTo)
			} else if h.Get("Reply-To") == "" {
				h.Set("Reply-To", msg.from)
			}
