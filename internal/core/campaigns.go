...
func normalizeTrackingConfig(raw json.RawMessage) json.RawMessage {
	if len(raw) == 0 {
...
func (c *Core) CreateCampaign(o models.Campaign, listIDs []int, mediaIDs []int) (models.Campaign, error) {
	...
	if err := c.q.CreateCampaign.Get(&newID,
		...
		o.ArchiveTemplateID,
		o.ArchiveMeta,
		normalizeTrackingConfig(o.TrackingConfig),
		pq.Array(mediaIDs),
		o.BodySource,
		o.ReplyTo,
		o.TrackingDomainID,
...
// QueryCampaignAnalyticsRecords returns paginated view or click records for campaigns.
func (c *Core) QueryCampaignAnalyticsRecords(campIDs []int, typ, fromDate, toDate string, offset, limit int) ([]models.CampaignAnalyticsRecord, int, error) {
	if !strHasLen(fromDate, 10, 30) || !strHasLen(toDate, 10, 30) {
	...
	switch typ {
	case "views":
		countStmt = c.q.QueryCampaignViewRecordsCount
		listStmt = c.q.QueryCampaignViewRecords
	case "clicks":
		countStmt = c.q.QueryCampaignClickRecordsCount
		listStmt = c.q.QueryCampaignClickRecords
	default:
		return nil, 0, echo.NewHTTPError(http.StatusBadRequest, c.i18n.T("globals.messages.invalidData"))
	}

	var total int
	if err := countStmt.Get(&total, pq.Array(campIDs), fromDate, toDate); err != nil {
		c.log.Printf("error counting campaign %s records: %v", typ, err)
		return nil, 0, echo.NewHTTPError(http.StatusInternalServerError,
			c.i18n.Ts("globals.messages.errorFetching", "name", "{globals.terms.analytics}", "error", pqErrMsg(err)))
	}

	out := []models.CampaignAnalyticsRecord{}
	if err := listStmt.Select(&out, pq.Array(campIDs), fromDate, toDate, limit, offset); err != nil {
		c.log.Printf("error fetching campaign %s records: %v", typ, err)
		return nil, 0, echo.NewHTTPError(http.StatusInternalServerError,
			c.i18n.Ts("globals.messages.errorFetching", "name", "{globals.terms.analytics}", "error", pqErrMsg(err)))
	}
...
func (c *Core) RegisterCampaignView(campUUID, subUUID string) error {
	if subUUID != "" {
		var found int
		if err := c.q.GetSubscriberIDByUUID.Get(&found, subUUID); err == sql.ErrNoRows {
			c.log.Printf("campaign view: subscriber uuid %s not found (campaign %s)", subUUID, campUUID)
		} else if err != nil {
			c.log.Printf("campaign view: error looking up subscriber uuid %s: %v", subUUID, err)
		}
	}
